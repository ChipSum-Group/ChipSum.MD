...
def cal_fft(lis1):
    ...
    if np.power(2,n1) == len(lis1):
        ...
        lis2 = [0.0]*n2
        lis3 = lis1[:] + lis2[:]
    fft_al = fft(lis3)[:int(N/2)]
    return fft_al.tolist()

def cal_sed(a_vel):
    fft_final = []
    for k in range(k_in_z):
        fft_x, fft_y, fft_z = [], [], []
        for i in range(n_cell):
            fft_vx = cal_fft(a_vel[0,i,:].tolist())
            fft_vy = cal_fft(a_vel[1,i,:].tolist())
            fft_vz = cal_fft(a_vel[2,i,:].tolist())
            ek = np.exp(1j*i*k/n_cell/k_in_z)
            fft_x.append(fft_vx)
            fft_y.append(fft_vy)
            fft_z.append((ek*np.array(fft_vz)).tolist())
        fft_xi = np.average(fft_x[:],axis=0)
        fft_yi = np.average(fft_y[:],axis=0)
        fft_zi = np.average(fft_z[:],axis=0)
        fft_ins = np.sqrt(np.abs(fft_xi)*np.abs(fft_xi)+np.abs(fft_yi)*np.abs(fft_yi)+np.abs(fft_zi)*np.abs(fft_zi))
        fft_final.append(fft_ins[:].tolist())
    return fft_final[:]
